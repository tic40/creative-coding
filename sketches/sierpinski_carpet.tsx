...
function carpet(n: number): Pattern {
  ...
  const res: Pattern = Array.from({ length: psize * 3 }, () => ".".repeat(psize * 3).split(''));
  for(let i = 0; i < 3; i++) for(let j = 0; j < 3; j++) if (i != 1 || j != 1) {
    for(let x = 0; x < psize; x++) for(let y = 0; y < psize; y++) {
...
export function draw(p: p5Types) {
  ...
  for(let i = 0; i < 40; i++) {
    ...
    p.rect((t % data.length) * size, Math.floor(t/data.length) * size,size,size)
    t++
  }
}
